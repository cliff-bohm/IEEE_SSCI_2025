...
void BerryWorld::runWorld(std::map<std::string, std::shared_ptr<Group>> &groups,
  ...
  std::shared_ptr<AbstractBrain> saveBrain;
  ...
  for (int whichMapIndex = 0; whichMapIndex < (int)whichMapsActual.size();
    ...
    for (auto evalGroup : evalGroups) {
      ...
      for (auto org : evalGroup) {
        ...
        if (visualize) {
            newHarvester->brain->setRecordActivity(true);
        }
        // set inital location
        auto pick =
  ...
  if (visualize) {
      auto WireBrain = std::dynamic_pointer_cast<WireCubeBrain>(saveBrain);
      std::cout << "score: " << groups[groupNameSpacePL->get(PT)]->population[0]->dataMap.getAverage("score") << std::endl;
      ...
      for (auto& i : WireBrain->wireCellsActive) {
          ...
          auto trimmed_cellStates = TS::trimTimeSeries(cellStates.back(), TS::Position::FIRST, lifeTimes, 1); // trim the first, this is the predictee
          auto trimmed_neighborStates = TS::trimTimeSeries(neighborStates.back(), TS::Position::LAST, lifeTimes, 1); // trim the last, this is the predictor
      ...
      FileManager::openAndWriteToFile("vis_output/cellToPredictorMap.csv", "map,\"" + cellToPredictorMap + "\"", "name, values");


      std::cout << "processed brain states size: " << brainStates.size() << std::endl;

      std::vector<int> worldState({ -1, 0 });
      std::vector<std::vector<int>> worldStates;

      // construct world state for next door direction based on seen
      // doorway signals this results in a true world state since 
      // the next door direction is dependent on agent action
      // this is read directly from the agent inputStates (i.e., sensors)
      for (auto& ws : WireBrain->inputStates) {
          if (ws[2] == 1) { // on sig "1"
              worldState[0] = 0;
          }
          if (ws[3] == 1) { // on sig "2"
          ...
          if (ws[1] == 0) { // we are either facing an opening, or in an opening. we can see what parts of the brain know when it's dealing with an opening!
              worldState[1] = 1;
          }
          else {
              worldState[1] = 0;
          ...
          worldStates.push_back(worldState);
      }

      std::cout << "WireBrain->inputStates.size(): " << WireBrain->inputStates.size() << std::endl;
      std::cout << "brainStates.size(): " << brainStates.size() << std::endl;
      ...
      if (1) {
          ...
          //TS::trimTimeSeries(cellStates.back(), TS::Position::FIRST, lifeTimes, 1)

          //FRAG::saveFragMatrix(TS::trimTimeSeries(worldStates, TS::Position::LAST, lifeTimes, 1), TS::trimTimeSeries(brainStates, TS::Position::FIRST, lifeTimes, 1), "vis_output/rawR_FragmentationMatrix.py", "feature", { "direction","open" }, 2, 0);
          FRAG::saveFragMatrix(worldStates,brainStates, "vis_output/rawR_FragmentationMatrix.py", "feature", { "direction","open" }, 2, 0);

          // new lets get frag for only the times we are "pretty sure" we are moving up or down
          //auto tempWS = TS::trimTimeSeries(worldStates, TS::Position::FIRST, lifeTimes, 1);
          //auto tempBS = TS::trimTimeSeries(brainStates, TS::Position::LAST, lifeTimes, 1);

          TS::intTimeSeries moveWS;
          TS::intTimeSeries moveBS;

          std::vector<TS::intTimeSeries> moveWS_decomposed(12);
          std::vector<TS::intTimeSeries> moveBS_decomposed(12);


          // the idea is to get make a new state space that only has transitions where we know that the agent is (should be) moving.
          // if worldstate[1] is 0 we are dealing with a doorway
          // we only count transitions if worldstate[1] is 0 (blocked front sensor) and has been for at least 2 updates - this should mean the agent is moving up or down
          int activeMoveStatus = 1000000; // this will hold updates indecies where front sensor is on and worldState[0] > -1

          for (int i = 0; i < worldStates.size(); i++) {
              if (worldStates[i][1] == 1) { // if front is open then we need to skip atleast
                  activeMoveStatus = 12 * 1 + 2; // 12 for each world update, plus 2 for this update
              }
              activeMoveStatus--; // count down

              if (0) { // report showing which states are collected
                  //std::cout << i << " " << i % 12 << " AMS: " << activeMoveStatus << "  " << ((activeMoveStatus <= 0) ? "**" : "") << std::endl;
                  std::cout << "    ";
                  for (auto& v : worldStates[i]) {
                      std::cout << v << " ";
                  }
                  std::cout << " :: ";
                  for (auto& v : brainStates[i]) {
                      std::cout << v << " ";
                  }
                  std::cout << std::endl;
              }

              if (activeMoveStatus <= 0) {
                  moveWS.push_back(worldStates[i]);
                  moveBS.push_back(brainStates[i]);

                  moveWS_decomposed[i % 12].push_back(worldStates[i]);
                  moveBS_decomposed[i % 12].push_back(brainStates[i]);
                  if (moveWS_decomposed[i % 12].back()[1] == 1) {
          ...
          std::cout << "Generating rawR_FragmentationMatrix_MOVE.py" << std::endl;
          FRAG::saveFragMatrix(moveWS, moveBS, "vis_output/rawR_FragmentationMatrix_MOVE.py", "feature", { "direction","open" }, 2, 0);
 
          std::cout << "Generating rawR_FragmentationMatrix_MOVE_STEPS.py" << std::endl;
          for (int i = 0; i < 12; ++i) {
              FRAG::saveFragMatrix(moveWS_decomposed[i], moveBS_decomposed[i], "vis_output/rawR_step_" + std::to_string(i) + "_FragmentationMatrix_MOVE.py", "feature", { "direction","open" }, 2, 0);
          }
      ...
      if (1) {
          ...
          for (int i = 0; i < 12; ++i) {
              ...
              for (int j = 0; j < n; ++j) {
                  brainStatesDecomposed[i][j] = brainStates[i + 12 * j];  // Group every nth vector with offset by i
                  worldStatesDecomposed[i][j] = worldStates[i + 12 * j];  // Group every nth vector with offset by i
              }
          }
          ...
          if (0) { // switch on to check that decompose works
              for (int i = 0; i < 12 * 5; i++) {
                  std::cout << i << ": ";
                  for (const auto& val : worldStates[i]) {
                      std::cout << " " << val;
                  }
                  std::cout << std::endl;
              }
          ...
          for (int i = 0; i < 12; ++i) {
              FRAG::saveFragMatrix(worldStatesDecomposed[i], brainStatesDecomposed[i], "vis_output/rawR_step_" + std::to_string(i) + "_FragmentationMatrix.py", "feature", { "direction","open" }, 2, 0);
          }
      }
